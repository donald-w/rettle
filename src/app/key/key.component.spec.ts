...
describe('KeyComponent', () => {
  ...
  beforeEach(async () => {
    ...
    gameEngineSpy = jasmine.createSpyObj<GameEngineService>('GameEngineService', ['registerKey', 'keyPressed']);
    gameEngineSpy.registerKey.and.returnValue(keyColor$.asObservable());
  ...
  it('should set classes based on key color stream', () => {
    ...
    keyColor$.next('light');
    fixture.detectChanges();
    expect(button.classList.contains('light')).toBeTrue();
    expect(button.classList.contains('dark')).toBeFalse();

    keyColor$.next('dark');
    fixture.detectChanges();
    expect(button.classList.contains('dark')).toBeTrue();
    expect(button.classList.contains('light')).toBeFalse();

    keyColor$.next('green');
    fixture.detectChanges();
    expect(button.classList.contains('green')).toBeTrue();

    keyColor$.next('yellow');
    fixture.detectChanges();
    // When already green, yellow should not override.
    expect(button.classList.contains('green')).toBeTrue();
    expect(button.classList.contains('yellow')).toBeFalse();
  });
